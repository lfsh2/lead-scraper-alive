...
class DataTable {
    ...
    formatCellValue(value, column, rowData, rowIndex) {
        ...
        if (column.type === 'actions') {
            ...
            const srcId = rowData._source?.id || this.options.campaignId || '';
            const srcIndex = rowData._source?.index ?? rowIndex;
            const phone = (rowData.phone || '').replace(/'/g, "\\'");
            const hasContent = rowData.intelligence?.marketingContent;
            const waBtn = phone ? `
                <button class="btn-vcard" onclick="dashboard.openWhatsApp('${phone}', ${hasContent ? JSON.stringify(rowData.intelligence.marketingContent.whatsapp || '').replace(/"/g, '&quot;') : "''"})" title="Open in WhatsApp">
            ...
            return `
                <div class="action-buttons">
                    <button class="btn-vcard" onclick="exportLeadVCard('${srcId}', ${srcIndex}, '${safeName}')" title="Export to Phone Contacts">
        ...
        if (column.type === 'link') {
            if (!actualValue) return '<span style="color:var(--text-muted)">—</span>';
            const safe = String(actualValue).replace(/"/g, '&quot;');
            let label = safe.replace(/^https?:\/\//, '').replace(/^www\./, '');
            if (label.length > 32) label = label.slice(0, 30) + '…';
            return `<a href="${safe}" target="_blank" rel="noopener" style="color:var(--accent);text-decoration:none">${label} ↗</a>`;
        }

        if (column.type === 'tags') {
            const arr = Array.isArray(actualValue) ? actualValue : [];
            if (!arr.length) return '<span style="color:var(--text-muted)">—</span>';
            return arr.map(t => `<span class="priority-badge" style="background:var(--bg-elevated);color:var(--text-secondary);margin-right:4px">${api.safeString(t)}</span>`).join('');
        }
